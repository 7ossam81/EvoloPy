    ...
    for l in range(0,Max_iter):
        ...
        for i in range(0,SearchAgents_no):
            ...
            for j in range (0,dim):     
                ...
                # JAYA Equation
                New_Position[j] = Positions[i][j]+r1*(Best_pos[j]-abs(Positions[i,j]))-r2*(Worst_pos[j]-abs(Positions[i,j]))
        ...
        if (l%1==0):
               print(['At iteration '+ str(l)+ ' the best fitness is '+ str(Best_score)])
