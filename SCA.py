    ...
    for l in range(0,Max_iter):
        ...
        for i in range(0,SearchAgents_no):
            for j in range (0,dim):     
                ...
                if (r4< (0.5)):
                    # Eq. (3.1)
                    Positions[i,j]= Positions[i,j]+(r1*numpy.sin(r2)*abs(r3*Dest_pos[j]-Positions[i,j]))
                else:
                    # Eq. (3.2)
                    Positions[i,j]= Positions[i,j]+(r1*numpy.cos(r2)*abs(r3*Dest_pos[j]-Positions[i,j]))
        ...
        if (l%1==0):
               print(['At iteration '+ str(l)+ ' the best fitness is '+ str(Dest_score)])
