...
import numpy as np
import pytest
from EvoloPy.optimizers.FFA import FFA
...
def test_FFA(sample_objective_function, sample_bounds):
    """Tests if the FFA algorithm runs correctly and produces a valid output."""
    lb, ub = sample_bounds
    dims = 5  # Number of dimensions
    PopSize = 10  # Population size
    iters = 30  # Number of iterations

    sol = FFA(sample_objective_function, lb, ub, dims, PopSize, iters)

    assert sol.bestIndividual.shape == (dims,), "Best individual should match input dimensions"
    assert np.array(sol.convergence).shape == (iters,), "Convergence curve should match iteration count"
